...
@ray.remote
def apply_rules_block(start_row, num_rows, state):
    s = state.size
    g = state.grid
    block = g[start_row:start_row+num_rows].copy()
    for n in range(num_rows):
        ...
        for j in range(s):
            ...
            num_live_neighbors = g[im1][jm1] + g[im1][j] + g[im1][jp1] + g[i][jm1] + g[i][jp1] + g[ip1][jm1] + g[ip1][j] + g[ip1][jp1]
...
class RayConwaysRules():
    ...
    def step(self, state):
        ...
        New implementation that parallizes execution.
        """
        num_rows_per_block = round(state.size / self.num_blocks)

        block_ids = [apply_rules_block.remote(block*num_rows_per_block, num_rows_per_block, state) for block in range(self.num_blocks)]
        blocks = ray.get(block_ids)
        new_grid = blocks[0]
        for m in range(1, len(blocks)):
            new_grid = new_grid+blocks(m)
...
""")
    ...
    for _ in range(4):
        time_ray_games4(num_games = 1, max_steps = args.steps, batch_size=50, grid_size=args.size, num_blocks = args.num_blocks)
